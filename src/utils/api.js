...
function mockFromImage(dataUrl) {
  ...
  return {
    prediction: top1,
    probs: [{ digit: top1, prob: 0.78 }, { digit: top2, prob: 0.17 }, { digit: top3, prob: 0.05 }],
    _mock: true,
  };
}
...
export async function predictDigit(dataUrl) {
  try {
    const res = await fetchWithTimeout(`${API}/predict`, {
      ...
      body: JSON.stringify({ image: dataUrl }), // expects data URL base64
    });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();

    // If backend returns { digit, proba }, map it to your UI shape:
    if ('digit' in data) {
      return {
    ...
    return mockFromImage(dataUrl); // fallback while backend is down
  }
}
